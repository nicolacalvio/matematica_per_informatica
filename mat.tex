...
La chiusura x (dove x è una qualunque proprietà es. riflessiva) di una relazione $R$ su un insieme $A$ è la più piccola relazione x che contiene $R$
\begin{itemize}
    \item \textbf{Riflessiva:} $R' = R \cup {(a,a) | a \in A}$
    \item \textbf{Simmetrica:} $R' = R \cup {(b,a)|(a,b) \in R}$
    \item \textbf{Transitiva:} $R' = R \cup {(a,c)|(a,b) \in R \lor (b,c) \in R}$
\end{itemize}
...
L'insieme delle parti di un insieme \(A\), noto anche come "potenza" di \(A\) e indicato con \(2^A\) o \(\mathcal{P}(A)\), è definito come l'insieme di tutti i possibili sottoinsiemi di \(A\), inclusi l'insieme vuoto \(\emptyset\) e l'insieme \(A\) stesso.

Per esempio, se abbiamo un insieme \(A = \{1, 2\}\), l'insieme delle parti di \(A\) sarà:
\[
\mathcal{P}(A) = \{\emptyset, \{1\}, \{2\}, \{1, 2\}\}
\]

In generale, se un insieme \(A\) ha \(n\) elementi, allora l'insieme delle parti di \(A\) avrà \(2^n\) elementi. Questo perché ogni elemento dell'insieme \(A\) può essere presente o assente in un sottoinsieme, il che dà due scelte (presente o assente) per ogni elemento, e quindi il numero totale di combinazioni possibili è \(2^n\).
...
\subsection{Teoremi Fondamentali - Bernstein - Schroeder}
\begin{itemize}
...
\subsection{Teorema di rappresentazione di Stone}
Per ogni algebra di Boole esiste uno spazio di Stone i cui elementi corrispondono agli elementi dell'algebra e le operazione algebriche di bool corrispondono a quelle insiemistiche. \\
Quindi in sostanza se una cosa è dimostrabile negli insiemi sicuramente è vera anche nelle algebre di Boole.

\subsection{Algebra di Heyting}
L'algebra di Heyting anche detta intuizionistica

\begin{itemize}
    \item \textbf{$\to$}: in Heyting l'implicazione non corrisponde a $\lnot A \lor B$
    \item \textbf{$\lnot$}: in Heyting è $bottom$ e non garantisce l'esistenza del complemento
\end{itemize}

\subsubsection{Conversione da Heyting a Boole}
Per convertire un algebra di Heyting in un algebra di Boole bisiogna rispettare le proprietà dell'algebra di Boole prestando attenzione particolare a 
\begin{itemize}
    \item \textbf{Legge del terzo escluso}: Per l'algebra di boole vale questa proprietà $P \lor \lnot P$ dove uno delle due è vero
    \item \textbf{Involuzione della negazione}: $\lnot \lnot P \equiv P$ 
\end{itemize}
...
Una dimostrazione è una sequenza di formule che contiene o deduzioni o istanze di assiomi o premesse, l'ultima della quale è la conclusione

\subsection{Conversione da op. logici a $\lnot$ $\to$}
$A \lor B = \lnot A \to B$ \\
$A \land B = A \to \lnot B$

\subsection{Correttezza e completezza}
\begin{itemize}
    \item \textbf{Correttezza}: proprietà di un sistema logico che garantisce che tutte le formule dimostrabili sono vere
    \item \textbf{Completezza}: proprietà di un sistema logico che tutte le formule vere sono dimostrabili
\end{itemize}
...
\textbf{Risposta}: La differenza $A-B$ sono gli elementi che appartengono ad $A$ ma non appartengono a $B$. \\ \\
\textbf{Domanda}: Come definisci l'intersezione tra due insiemi $A$ e $B$? \\
\textbf{Risposta}: L'intersezione è le $X \in A: X \in B$ ossia appartenenti sia ad $A$ che a $B$(principio di comprensione) \\ \\
...
\textbf{Risposta}: dimostrazione
\begin{itemize}
    \item $A - (B \cup C) \subseteq (A - B) \cap (A-C)$ quindi prendendo una $x \in A$ e $x \notin B \cup C \to x \notin B, x \notin C$ quindi $x \in (A-B) \cap (A-C)$ quindi ogni elemento di $A-(B \cup C)$ è anche in $(A-B) \cap (A-C)$
    \item $(A-B) \cap (A-C) \subseteq A - (B \cup C)$
    prendiamo una $x \in (A-B) \cap (A-C) \to x \in A, x \notin B, C, B \cup C$ quindi $x \in A - (B \cup C)$
\end{itemize}
...
\textbf{Risposta}: La chiusura transitiva della funzione successore è l'insieme delle coppie $(x,y)$ in cui $x$ < $y$. Esempio: ${(1,2), (2,3)} \to {(1,2),(2,3),(1,3),(1,4)}$\\ \\
\textbf{Domanda}: Qual è il più piccolo sovrainsieme della funzione successore che ha la proprietà di essere una relazione transitiva? \\
\textbf{Risposta}: Minore stretto perchè è il sovrainsieme più grande $T={(a,b) \in N \times N | a < b}$\\ \\
...
\textbf{Risposta}: Riflessività, simmetria, transitività \\ \\
...
\textbf{Risposta}: No, posso concludere che una delle due è iniettiva\\ \\
\textbf{Domanda}: Dammi una relazione transitiva e una non transitiva sugli insiemi dei booleani. \\
\textbf{Risposta}: L'implicazione è una relazione transitiva nei booleani, la non transitiva non esiste\\ \\
...
\textbf{Risposta}: $A = \mathbb{N}$
$f(n)=\begin{cases} 0, & \mbox{se }n\mbox{ x=0} \\ 1, & \mbox{se }n\mbox{ x\textgreater 0}
\end{cases}$ \\ \\
...
Bisogna quindi trovare due inizioni per la biezione. Nel primo lato ossia $\mathbb{N} \to$ lista di $\mathbb{N}$ possiamo mappare i naturali in singleton $x\to {x}$. Nel secondo lato possiamo usare la \textbf{codifica di G\"odel} per mappare una lista in un intero creando così una inizieone. Un esempio di questa codifica è ${2,3,5}\to 30$ poichè $2^1 \times 3^1 \times 5^1 = 30$ ossia per generare l'intero usiamo i numeri primi alla potenza delle volte che appaiono e gli moltiplichiamo fra loro. Nel caso non ci fossero numeri primi faremo per ogni intero la scomposizione in numeri primi\\ \\
\textbf{Domanda}: Sono più i numeri naturali o i sottoinsiemi finiti dei naturali? \\
\textbf{Risposta}: $|\mathbb{N}| = \aleph_0$ (aleph-zero) ossia la cardinalità più piccola dell'infinito (anche detto infinito numerabile). $|P(\mathbb{N})|\textgreater \aleph_0$ la cardinalità dell'insieme delle parti di un insieme è $2^{|A|} = 2^{\aleph_0}$\\ \\
...
\textbf{Risposta}: Sì, è visualizzabile con la diagonale di Cantor, per ogni coppia di numeri naturali possiamo associare un numero naturale otteniamo quindi una biiezione\\ \\
\textbf{Domanda}: Capire se $N$ e $N^*$ (stella di kleene) sono equipotenti. \\
\textbf{Risposta}: $N^*$ ha una cardinalità maggiore poichè rappresenta l'insieme di tutte le sequenze finite\\ \\
\textbf{Domanda}: Secondo te ci sono più elementi nell'insieme $bool \times \mathbb{N}$ ossia il prodotto cartesiano tra (true,false) e i naturali o le funzioni che vanno dai booleani ai naturali? \\
\textbf{Risposta}: $bool \times \mathbb{N}$ è inifito, è l'insieme di tutte le coppie di cui il primo elemento è booleano e il secondo naturale, le funzioni da bool a nat sono anche esse infinite.
In conclusione vediamo come possiamo mappare allo stesso modo i valori $bool \times nat$ e i valori $bool \to \mathbb{N}$ quindi essi sono equipotenti.\\ \\
...
\begin{prooftree}
    \AxiomC{} % Normally, we start with axioms or assumptions, but in this case, the tree is very simple.
    \UnaryInfC{$A \rightarrow (A \rightarrow A)$} % Assioma 1: A → (A → A)
    \UnaryInfC{$A \rightarrow A$} % Applying implication: From A, infer A
\end{prooftree}
...
\textbf{Risposta}: no non può esistere deve essere $2^n$\\ \\
...
\textbf{Risposta}: è un assioma di Hilbert quindi è dimostrata
...
\begin{prooftree}
    \AxiomC{$\vdash \lnot A \to (B \to \lnot A)$}
    \UnaryInfC{$\lnot A \vdash B \to \lnot A$}
    \UnaryInfC{$\lnot A \vdash A \to \lnot B$}
    \UnaryInfC{$\lnot A \to (A \to \lnot B)$}
    \UnaryInfC{$\lnot(A \to \lnot B) \to A$}
    \UnaryInfC{$A \land B \to A$}
\end{prooftree}
...
\newpage
\section{Esame Orale}
Indovinello: sei in un corridoio che termina con due porte, una port conduce alla salvezza mentre l'altra a morte certa. Ci sono due guardiani, uno dice sempre la verità e l'altro mente sempre. Hai il diritto di fare solo una domanda a uno solo dei guardiani per scoprire quale porta conduce alla salvezza ma non sai quale guardiano dice la verità e quale mente
\subsection{Soluzione}
bisogna chiedere "se chiedessi all'altro guardiano quale porta conduce alla salvezza cosa mi direbbe?" In questo modo in entrambi i casi la risposta sarebbe la porta che conduce alla morte
\subsection{Principio di non contraddizione}
questo indovinello gioca sull'idea di verità e menzogna $(P, \lnot P)$ dove bisogna ragionare per trovare una risposta affidabile.
L'indovinello sottolinea l'impossibilità logica per il guardiano che dice sempre la verità di mentire e viceversa riflettendo l'idea che una proposizione e la sua negazione non possono coesistere senza contradizione.
\subsection{Dimostrazione del principio di non contraddizione con Hilbert}
$\lnot(P \land \lnot P)$ che possiamo riscrivere come $P \to \lnot(\lnot P)$
per dimostrarlo necessitiamo della regola della doppia negazione che quindi dimostreremo con Hilbert
\subsection{Dimostrazione della regola della doppia negazione}
\begin{prooftree}
    \AxiomC{${\lnot \lnot A} \vdash \lnot \lnot A \to (\lnot \lnot \lnot \lnot \to \lnot \lnot A)$ Assioma 1} 
    \UnaryInfC{${\lnot \lnot A} \vdash \lnot \lnot A$ Ipotesi} 
    \UnaryInfC{${\lnot \lnot A} \vdash \lnot \lnot \lnot \lnot A \to \lnot \lnot A$ Modus Ponens}
    \UnaryInfC{${\lnot \lnot A} \vdash \lnot A \to \lnot \lnot \lnot A$ Contrapp.} 
    \UnaryInfC{${\lnot \lnot A} \vdash \lnot \lnot A \to A$ Contrapp.} 
    \UnaryInfC{${\lnot \lnot A} \vdash A$ Modus Ponens} 
    \UnaryInfC{$\lnot \lnot A \to A$}
\end{prooftree} 
Quindi possiamo dimostrare per Modus Ponens che $P \to \lnot (\lnot P)$ è vero dimostrando quindi il principio di non contraddizione.
\end{document}
