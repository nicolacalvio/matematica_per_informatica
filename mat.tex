...
In conclusione vediamo come possiamo mappare allo stesso modo i valori $bool \times nat$ e i valori $bool \rightarrow nat$ quindi essi sono equipotenti.\\ \\
...
\textbf{Domanda}: Il seguente enunciato $(A \rightarrow \lnot A) \rightarrow false$ è vero quando\\
\textbf{Risposta}: 
\begin{itemize}
    ...
    \item è sempre vero
\end{itemize}
La risposta corretta è che l'enunciato $(A \rightarrow \lnot A) \rightarrow false$ è vera quando $A$ è vero, perchè l'implicazione $A \rightarrow \lnot A$ in questo caso sarebbe false, rendendo vero l'enunciato dato secondo le regole dell'implicazione. \\ \\
\textbf{Domanda}: $(A \land B) \rightarrow [C \rightarrow (A \land C)]$ dimostralo \\ 
\textbf{Risposta}: traduco in formule logicamente equivalenti con not e implica $A \lor B \equiv \lnot A \rightarrow B$ e $A \land B \equiv \lnot (A \rightarrow \lnot B)$.
Dimostriamo quindi creando due lemmi $A \land B \rightarrow A$ e $A \rightarrow [C \rightarrow (A \land C)]$.
Se riusciamo a dimostrare questi due lemmi per la proprietà transitiva $(A \land B) \rightarrow [C \rightarrow (A \land C)]$.
Dimostriamo quindi $A \land B \rightarrow A$ e la riscriviamo come $\lnot(A \rightarrow \lnot B) \rightarrow A$.
\begin{equation}
    \dfrac{ \dfrac{\lnot A \rightarrow (A \rightarrow \lnot B)}{\lnot (A \rightarrow \lnot B) \rightarrow A }}{A \land B \rightarrow A}
\end{equation}
Abbiamo quindi inverito l'implica negando le due proposizioni. Adesso applichiamo la deduzione. \\
$\lnot A \vdash A \rightarrow \lnot B$, giro di nuovo negando: $\lnot A \vdash B \rightarrow \lnot A$. riapplichiamo teorema di deduzione. $\vdash \lnot A \rightarrow (B \rightarrow \lnot A)$ che non ho bisogno di dimostrare poiché primo assioma di Hilbert.
 \\ \\
\end{document}
